...
fun interface SwipeConstraint {
    ...
    fun classify(deltaX: Float, deltaY: Float): SwipeDirection? {
        ...
        return classify(angle(deltaX, deltaY))
    }

    companion object {
        fun angle(delta: Offset) = angle(delta.x, delta.y)
        fun angle(delta: Velocity) = angle(delta.x, delta.y)
        fun angle(deltaX: Float, deltaY: Float) = ((atan2(deltaY, deltaX) * 180f / piFloat) + 360f) % 360f
        ...
        fun angle(
            ...
            require(tolerance.isFinite() && tolerance in 0f..360f) { "tolerance must be in 0f..360f, was $tolerance" }
            return SwipeConstraint { angle ->
                if (shortestDiff(angle, angleDegrees) <= (tolerance / 2)) direction else null
            }
        }

        fun horizontal(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = multiDirectionDefaultTolerance): SwipeConstraint {
            require(tolerance.isFinite() && tolerance in 0f..360f) { "tolerance must be in 0f..360f, was $tolerance" }
        ...
        fun vertical(tolerance: Float = multiDirectionDefaultTolerance): SwipeConstraint {
            require(tolerance.isFinite() && tolerance in 0f..360f) { "tolerance must be in 0f..360f, was $tolerance" }
            return SwipeConstraint { angle ->
        ...
        fun fourWay(
            ...
            tolerance: Float = multiDirectionDefaultTolerance
        ): SwipeConstraint {
            require(tolerance.isFinite() && tolerance in 0f..360f) { "tolerance must be in 0f..360f, was $tolerance" }
            val startAngle = if (layoutDirection == LayoutDirection.Ltr) 180f else 0f
            val endAngle = if (layoutDirection == LayoutDirection.Ltr) 0f else 180f
        ...
        fun all(layoutDirection: LayoutDirection = LayoutDirection.Ltr): SwipeConstraint {
            val startAngle = if (layoutDirection == LayoutDirection.Ltr) 180f else 0f
            val endAngle = if (layoutDirection == LayoutDirection.Ltr) 0f else 180f
        ...
        fun single(
            direction: SwipeDirection,
            layoutDirection: LayoutDirection = LayoutDirection.Ltr,
            tolerance: Float = singleDirectionDefaultTolerance
        ): SwipeConstraint {
        ...
        fun start(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.Start, layoutDirection, tolerance)

        fun end(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.End, layoutDirection, tolerance)

        fun top(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.Top, layoutDirection, tolerance)

        fun bottom(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.Bottom, layoutDirection, tolerance)

        fun topStart(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.TopStart, layoutDirection, tolerance)

        fun topEnd(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.TopEnd, layoutDirection, tolerance)

        fun bottomStart(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.BottomStart, layoutDirection, tolerance)

        fun bottomEnd(layoutDirection: LayoutDirection = LayoutDirection.Ltr, tolerance: Float = singleDirectionDefaultTolerance): SwipeConstraint =
            single(SwipeDirection.Companion.BottomEnd, layoutDirection, tolerance)
