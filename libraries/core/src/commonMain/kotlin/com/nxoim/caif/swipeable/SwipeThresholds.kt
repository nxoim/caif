...
data class SwipeThresholds(
    ...
    init {
        require(activation == null || activation.value >= 0f) {
            "activation must be null or non-negative"
        }
        require(confirmationMinDistance.value >= 0f) {
            "confirmationMinDistance must be non-negative"
        }
        require(confirmationVelocity.value >= 0f) {
            "confirmationVelocity must be non-negative"
        }
        require(offAxisCancellation.value >= 0f) {
            "offAxisCancellation must be non-negative"
        }
    }
